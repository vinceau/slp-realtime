...
import { pipeFileContents, SlippiGame, SlpRealTime, RxSlpStream, ComboFilter, Character } from "../src";
...
describe("combo calculation", () => {
  ...
  describe("when filtering netplay names", () => {
    ...
    it("can match combos from Smashladder SLP files", async () => {
      ...
      const slpStream = new RxSlpStream(undefined, { suppressErrors: false });
      const realtime = new SlpRealTime();
      realtime.setStream(slpStream);
